...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'no-turning-back':
				dialogue = CoolUtil.coolTextFile(Paths.txt('no-turning-back/ntbDialogue'));
			case 'purple-trauma':
				dialogue = CoolUtil.coolTextFile(Paths.txt('purple-trauma/ptDialogue'));
			case 'demons-lair':
				dialogue = CoolUtil.coolTextFile(Paths.txt('demons-lair/dlDialogue'));
		}
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'no-turning-back':
				...
				var bg:BGSprite = new BGSprite('WHG/blueback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('WHG/bluefront'));
			...
			case 'purple-trauma':
				...
				var bg:BGSprite = new BGSprite('WHG/purpleback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('WHG/purplefront'));
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				stageFront.antialiasing = true;
				stageFront.scrollFactor.set(0.9, 0.9);
			...
			case 'demons-lair':
				...
				var bg:BGSprite = new BGSprite('WHG/redback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('WHG/redfront'));
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				stageFront.antialiasing = true;
				stageFront.scrollFactor.set(0.9, 0.9);
		...
		if (isStoryMode && !seenCutscene)
		{
			...
			switch (curSong.toLowerCase())
			{
				...
				case 'senpai' | 'roses' | 'thorns'| 'no-turning-back' | 'purple-trauma' | 'demons-lair':
					schoolIntro(doof);
				case 'ugh':
					ughIntro();
				case 'stress':
	...
	override function beatHit()
	{
		...
		if (curBeat == 384 && curSong == 'demons-lair')
		{
			FlxG.camera.fade(FlxColor.BLACK, 0.01, true);
			aprilfooled = new FlxText((Std.int(FlxG.width / 2)) -270, Std.int(FlxG.height / 2), 0, "HAPPY APRIL FOOLS!");
			//aprilfooled.screenCenter(X);
			aprilfooled.screenCenter(Y);
			aprilfooled.scrollFactor.set();
			aprilfooled.setFormat(Paths.font("vcr.ttf"), 48, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			camHUD.visible = false;
			var blackShit:FlxSprite = new FlxSprite(-FlxG.width * FlxG.camera.zoom,
			-FlxG.height * FlxG.camera.zoom).makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
			blackShit.scrollFactor.set();
