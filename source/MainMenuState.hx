...
{
	var secretKey:Array<FlxKey> = [FlxKey.S, FlxKey.E, FlxKey.C, FlxKey.R, FlxKey.E, FlxKey.T];
	...
	override function update(elapsed:Float)
	{
		...
		if(finalKey != FlxKey.NONE) {
			...
			if(lastKeysPressed.length > secretKey.length)
			{
			...
			if(lastKeysPressed.length == secretKey.length)
			{
				...
				for (i in 0...lastKeysPressed.length) {
					if(lastKeysPressed[i] != secretKey[i]) {
						isDifferent = true;
						break;
					}
					else
					{
						FlxG.sound.play(Paths.sound('clicky'));
					}
				}

				if(!isDifferent) {
					Application.current.window.alert("when all is done\n\nkeep moving forward", "FIWHG");
